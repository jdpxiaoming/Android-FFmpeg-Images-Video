...
const char * const forced_keyframes_const_names[] = { "n", "n_forced",
		"prev_forced_n", "prev_forced_t", "t", NULL };
	...
	for (i = 0; i < nb_frames; i++) {
		...
		if (s->oformat->flags & AVFMT_RAWPICTURE
			...
			if (enc->coded_frame->interlaced_frame)
				enc->field_order =
						enc->coded_frame->top_field_first ?
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		while (1) {
			...
			case AVMEDIA_TYPE_AUDIO:
				avfilter_copy_buf_props(filtered_frame, picref);
				filtered_frame->pts = frame_pts;
				if (!(ost->st->codec->codec->capabilities
						& CODEC_CAP_PARAM_CHANGE)
						&& ost->st->codec->channels
...
static void do_streamcopy(InputStream *ist, OutputStream *ost,
	...
	if (ost->st->codec->codec_type
			== AVMEDIA_TYPE_AUDIO&& pkt->dts != AV_NOPTS_VALUE) {int duration = av_get_audio_frame_duration(ist->st->codec, pkt->size);
	if(!duration)
...
static int init_input_stream(int ist_index, char *error, int error_len) {
	...
	if (ist->decoding_needed) {
		...
		ist->dr1 = (codec->capabilities & CODEC_CAP_DR1)
				&& !(FF_API_DEINTERLACE && do_deinterlace);
...
static int process_input(int file_index) {
	...
	if (debug_ts) {
		av_log(NULL, AV_LOG_INFO,
				...
				av_get_media_type_string(ist->st->codec->codec_type),
	...
	if (!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE
		...
		if (ist->next_dts == AV_NOPTS_VALUE
			...
			for (i = 0; i < is->nb_streams; i++) {
				...
				if (st->discard
						== AVDISCARD_ALL|| st->start_time == AV_NOPTS_VALUE)continue;
				new_start_time =
						FFMIN(new_start_time, av_rescale_q(st->start_time, st->time_base, AV_TIME_BASE_Q));
			}
...
int video_merge(int argc, char **argv) {
...
jstring Java_com_catcry_ffmpeg_FFmpegJNITools_fenGe(JNIEnv* env, jobject thiz) {
...
//    arg[0]="ffmpeg";
//    arg[1]="-i";
//    arg[2]="/storage/sdcard0/video2.avi";
//    arg[3]="-i";
//    arg[4]="/storage/sdcard0/333.mp3";
//    arg[5]="-vcodec";
//    arg[6]="copy";
//    arg[7]="-acodec";
//    arg[8] = "copy";
//    arg[9]="/storage/sdcard0/output1.avi";
	...
	return (*env)->NewStringUTF(env, "完成");
}

int fenGe() {
	AVFormatContext *pFormatCtx = NULL;
	int err, i;
	char *filename = "/storage/sdcard0/video2.avi"; // argv[1];
	AVCodec *pCodec = NULL;
...
void SaveFrame(AVFrame *pFrame, int width, int height, int iFrame) {
	...
	snprintf(szFilename, 255, "/storage/sdcard0/%03d.ppm", iFrame);
...
jstring Java_com_catcry_ffmpeg_FFmpegJNITools_test(JNIEnv* env, jobject thiz,
		jstring str, jint i) {
...
//	arg[4] = "10";
//	arg[5] = "-i";
//	arg[6] = "/storage/sdcard0/%03d.ppm";
//	arg[7] = "-i";
//	arg[8] = "/storage/sdcard0/222.mp3";
//	arg[9] = "-absf";
//	arg[10] = "aac_adtstoasc";
//	arg[11] = "/storage/sdcard0/output3.mp4";
	__android_log_print(ANDROID_LOG_DEBUG, "test", "============");
//	    int a = 8;
//	            char *arg[10];
//	            arg[0] = "ffmpeg";
//	            arg[1] = "-i";
//	            arg[2] = "/storage/sdcard0/%03d.ppm";
//	            arg[3] = "-i";
//	            arg[4] = "/storage/sdcard0/222.mp3";
//	            arg[5] = "-s";
//	            arg[6] = "480x320";
//	            arg[7] = "-vcodec";
//	            arg[8] = "mpeg4";
//	            arg[9] = "/storage/sdcard0/output3.mp4";
//
//	            int ret = video_merge(a, arg);

getMp4();
	return (*env)->NewStringUTF(env, "haah");
}
...
int getMp4()
{
    ...
    filename = "/storage/sdcard0/output3.mp4";
